...
def _run_cli() -> None:
    ...
    if args.command is None:
        ...
        echo()
        if ENV_FILE.is_file():
            echo(f"  {G}✓ Configuration found.{NC}")
            echo(f"  {D}Config file:{NC} {ENV_FILE}")
            echo()
        else:
            echo(f"  {Y}Not configured yet.{NC}")
            echo(f"  {D}Get started in 3 steps:{NC}")
            echo(f"    {C}1.{NC} gaet init          Setup wizard (local + cloud DB)")
            echo(f"    {C}2.{NC} gaet push          Backup local -> cloud")
            echo(f"    {C}3.{NC} gaet status        Show sync status")
            echo()

        echo(f"  {B}Popular commands:{NC}")
        echo(f"    {C}gaet init{NC}           Setup database")
        echo(f"    {C}gaet push{NC}           Backup to cloud")
        echo(f"    {C}gaet fetch{NC}          Restore from cloud")
        echo(f"    {C}gaet status{NC}         Check sync status")
        echo(f"    {C}gaet check{NC}          Validate connection")
        echo(f"    {C}gaet serve{NC}          Open web dashboard")
        echo()
        echo(f"  {D}Need help?{NC}")
